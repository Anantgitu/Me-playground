...
const Skills = ({ skills }) => {
    ...
    const getSkillBadgeColor = (level) => {
        if (level >= 8) return 'bg-green-900 text-green-200';
        if (level >= 6) return 'bg-blue-900 text-blue-200';
        if (level >= 4) return 'bg-yellow-900 text-yellow-200';
                    ...
                    {skills.map((skill) => (
                        <div key={skill._id || skill.name} className="flex items-center justify-between">
                            <div className="flex items-center space-x-3 flex-1">
                                <span className="font-medium text-gray-200 min-w-0 flex-1">{skill.name}</span>
